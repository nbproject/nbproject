...
(function($) {
    var V_OBJ = $.extend({},$.ui.view.prototype,{
		...
		self._setData("pages", {}); //pages that have been rendered at current resolution
		self._setData("page", null); 
		self._setData("id_source", null);
		...
		self.element.addClass("docView").scroll(function(evt){
			...
			if (self._getData("in_page_transition")==false){
			    timerID = window.setTimeout(function(){
				    ...
				    var h = self._getData("h")+self._getData("v_margin");
		...
		var width0	= model.o.file[id_source].w;
		//var width0	= 612; //SACHA: FIXME - should check it's really the case ! 

		//		var height0	= 792;
		var height0	= model.o.file[id_source].h;
		...
		if ( res0 != self._getData("resolution") || scale != self._getData("scale")){
		    ...
		    self._setData("h", h);
		    var src="";
		    for (var i=1;i<=model.o.file[id_source].numpages;i++){
			style = "width: "+w+"px;height: "+h+"px";
			contents+="<div class='material'  page='"+i+"' style='"+style+"' ><div class='selections'/><div class='links'/><img class='material' page='"+i+"'/></div>";
		    }
		    ...
		    self._setData("v_margin", parseInt($material.css("margin-bottom")) +  parseInt($material.css("margin-top")) );
		...
		this._render_one(p);
		if (p<this._getData("model").o.file[this._getData("id_source")].numpages){
		...
		var self	= this;
		var pages	= self._getData("pages");
		//		var page	= self._getData("page");
		if (page in pages){
		    return;
		}
		pages[page]	= true;
		...
		var self	= this;
		var res		= self._getData("resolution");
		var scale	= self._getData("scale");
		var id_source	= self._getData("id_source");
		var w		= self._getData("w");
		var h		= self._getData("h");
		var style	= "width: "+w+"px;height: "+h+"px";
		var src_pref	= self.options.img_server+"/pdf/cache2/"+res+"/"+scale+"/"+id_source+"?invite_key="+self.options.invite_key+"&amp;page=";
