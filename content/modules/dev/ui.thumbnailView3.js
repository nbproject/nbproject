...
(function($) {
    ...
    var V_OBJ = $.extend({},$.ui.view.prototype,{
		...
		let controls = $("<div class='panel controls'><table><tr class='control-row'><td><div class='button' action='page' direction='previous' title='Go to previous page'><img class='buttonimg' src='/content/data/icons/png/go_previous_24.png'/></div></td><td colspan='2'><span class='indicator'><span class='currentpage'>5</span>/<span class='numpages'>5</span></span></td><td><div class='button' action='page' direction='next' title='Go to next page'><img class='buttonimg' src='/content/data/icons/png/go_next_24.png'/></div></td></tr><tr class='control-row'><td><div class='button' action='zoom' direction='out' title='Zoom out'><img class='buttonimg' src='/content/data/icons/png/zoomout_24.png'/></div></td><td colspan='2'> <span class='indicator zoomlevel'>100%</span></td><td> <div class='button' direction='in' action='zoom' title='Zoom in'><img class='buttonimg' src='/content/data/icons/png/zoomin_24.png'/></div></td></tr><tr class='control-row'><td><div class='button' action='visibility' status='on' title='Toggle notes visibility'><img class='buttonimg' /></div></td><td><div class='button' action='global_editor' title='Add a comment on the whole file'><img class='buttonimg' src='/content/data/icons/png/pen_24.png'/></div></td><td><div class='button' action='download' title='Download this PDF'><img class='buttonimg' src='/content/data/icons/png/save_24.png'/></div></td></tr></table></div> ");
		...
		$(".button[action=download]", controls).click(function(evt){
		    window.open( self.options.img_server+"/pdf/repository/"+$.concierge.get_state("file")+"?invite_key="+self.options.invite_key);
		});
		...
		case "visibility":
		    ...
		    $("div.button[action=visibility]>img.buttonimg", self.element).attr("src","/content/data/icons/png/"+filename);
		    break;
		}
	    }, 
		...
		for (let i =1;i<=numpages;i++){
		    s += "<li class='thumbnail' ><div class='thumbnail-contents' id_item='" + i+"' ><img src='"+ self.options.img_server+"/pdf/cache2/72/20/"+current_file+"?invite_key="+self.options.invite_key+"&amp;page="+i+"'/></div></li>";
		}
		$w.html(s);
