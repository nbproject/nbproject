...
(function($) {
    ...
    console.log(self.options);
		...
		var section_tag_option = ""; //hack to hide tags
                ...
                //var fun = "<script> function add_tag(type) {if(type==0) {window.alert('curious');} else if(type==1) {window.alert('confused');} else if(type==2) {window.alert('useful');} else {window.alert('interested');}} </script>";
                      ...
                      for(i=0; i<checkTags.length; i++)
                      {
                          if(checkTags[i].charAt(0) === "#" && knownTags.indexOf(checkTags[i]) > -1)
                          {
                              if(currentValue.indexOf(checkTags[i]) <= -1)
                              {
                                  ...
                                  if(bt.className === "clicked") {
                                      bt.className = "unclicked";
                                      srcLen = bt.src.length;
                                      tmpsrc = bt.src.substring(0, srcLen - 7);
                                      bt.src = tmpsrc + '.png';
                                  }
                              }
                          }
                      ...
                      for(i=0; i<checkTags.length && !deleteEvent; i++)
                      {
                          if(checkTags[i].charAt(0) === "#" && knownTags.indexOf(checkTags[i]) > -1)
                          {
                              if(lastValue.indexOf(checkTags[i]) <= -1)
                              {
                                  tagId = checkTags[i].substr(1) + "Id";
                                  bt =document.getElementById(tagId);
                                  if(bt.className === "unclicked") {
                                      bt.className = "clicked";
                                      srcLen = bt.src.length;
                                      tmpsrc = bt.src.substring(0, srcLen - 4);
                                      bt.src = tmpsrc + '_on.png';
                                  }
                              }
                          }
                      }
