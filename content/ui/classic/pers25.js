...
NB.pers.on_getStore = function(payload){
    ...
    */
	...
	let favorite_msg = "<span class='caption'>No favorite notes yet...  <a href='/pdf/glossary.xhtml#item-favorite'>(What's this?)</a></span> <br/>";
	if (E[j].admin==1){
	...
	if (S2[j].locs_unanswered != 0){
	    unanswered_msg = "<a href='?t=p24&amp;view=unanswered&amp;id_ensemble="+j+"'>"+S2[j].unanswered+"</a> <span class='caption'> replies are being requested on </span> <a href='?t=p24&amp;view=unanswered&amp;id_ensemble="+j+"'>"+S2[j].locs_unanswered+"</a> <span class='caption'>threads</span><br/>";
	}
	if (S2[j].auth != 0){
	    auth_msg = "<a href='?t=p24&amp;view=newauth&amp;id_ensemble="+j+"'>"+S2[j].newauth+"</a> <span class='caption'> unseen replies related to my </span> <a href='?t=p24&amp;view=auth&amp;id_ensemble="+j+"'>"+S2[j].auth+"</a> <span class='caption'>comments</span><br/>";
	}
	if (S2[j].favorite != 0){
	    favorite_msg = "<a href='?t=p24&amp;view=newfavorite&amp;id_ensemble="+j+"'>"+S2[j].newfavorite+"</a> <span class='caption'> unseen replies related to my </span> <a href='?t=p24&amp;view=favorite&amp;id_ensemble="+j+"'>"+S2[j].favorite+"</a> <span class='caption'> <em>favorite</em> comments</span><br/>";
	}
...
NB.pers.onsearch = function(id_ensemble){
    ...
    //    $("#search_dialog_ensemble").html("<option id_ensemble='"+id_ensemble+"'>"+NB.pers.store.o.ensemble[id_ensemble].name+"</option>").attr("disabled", "disabled");
...
NB.pers.preSearchPage = function(){
    ...
    $.concierge.get_component("presearch")({"id_ensemble": ids_ensemble, "text": search_text, page_offset: page_offset}, function(payload){
	    ...
	    /*
	      if (payload.total == 0){
	      $("#search-dialog-collagelink").hide();
	      } 
	      else{
	      $("#search-dialog-collagelink").show().attr("href", "/?t=p24&amp;view=search&amp;id_search="+payload.id_search);
	      }
	    */
...
NB.pers.call = function(fctname, dict, callback, nowait){
    ...
    NB.rpc.rpc_json("pdf3/rpc", fctname,[dict],NB.rpc.__callback, {"cb": callback});
};
...
NB.pers.proceedUpload = function(payload){
    ...
    form.setAttribute("action", "/pdf3/upload?id_ensemble="+NB.pers.currentEnsemble+"&id_source="+ payload.id_source);
    form.submit();
