...
NB.pdf.onReady = function(event){
    //    $(".embedded").ready(NB.pdf.onEmbeddedReady);
    NB.debug("in ready() !");
...
NB.pdf.loadPage = function(){
	...
	for (i=0;i<numx;i++){
	    ...
	    left = 256*i;
	    top = 256*j;
	    //SACHA FIXME: unsafe this lets anyone be able to view anoyone's file
...
NB.pdf.getPosition = function(elt){
    ...
    */
    //cleaner !
    p.top = parseInt(elt.style.top);
    p.left = parseInt(elt.style.left);
    return p;
};
