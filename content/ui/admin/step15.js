...
NB.pers.init = function(){
    ...
    $("#view-1").filesView({img_server: NB.conf.servers.img, invite_key: NB.conf.identity, admin: false});
    $("#view-2").treeView({admin:false});
    ...
    $.concierge.addFactory("file", "doc_viewer", function(id){
	    //	    $.mods.ready("docview", function(){
		    ...
		    var pers_id		= $vp.viewport("newView", "tab_"+id,$.E(NB.pers.store.o.file[id].title), {show: function(){$.concierge.trigger({type: "file", value:  id});}});
...
NB.pers.createStore = function(payload){
    ...
    $.concierge.setHistoryHelper(function(payload, cb){NB.pers.call("log_history", payload, cb);}, 60000);
