...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask( "cssmin", "Minify CSS files with Sqwish.", function() {
    ...
    if (servername.length){
        console.log("compiling css for "+servername);
    }
    //use resource URLs based on location of compilied CSS file
    grunt.file.expandFiles( this.file.src ).forEach(function(file){
            ...
            if (servername.length){
                //expand resources URLs to include server name
                current = current.replace(/url\((?!'?"?https?:\/\/.*)'?"?\/(.*)/g, "url("+servername+"$1");    //'")); //replace absolute urls
                current = current.replace(/url\((?!'?"?https?:\/\/.*)'?"?(.*)/g, "url("+servername+srcPaths.slice(0,-1).join("/")+"/$1");    //'")); //replace relative urls

                
                //srcPaths.slice(0,-1).join("/")+
                current = current.replace(/url\(([^'"]*)("|')\)/g, "url($1)");  //));//remove trailing quotes we may have left.                
            }
            else{        
                //use resource URLs based on location of compilied CSS file
                var maxCommonIndex = findMaxCommonIndex(srcPaths, destPaths);
                var replacementString = dots(destPaths.length - 2) + srcPaths.slice(maxCommonIndex+1, -1).join("/")+"/" ;
                //extra //'s below to fix emacs syntax highlighting...
